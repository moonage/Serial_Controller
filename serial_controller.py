...
class SerialReadThread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            if buf:
                #print(buf)  
                buf = bytes("Received : ", "utf-8") + binascii.hexlify(buf) + bytes([0x0A]) # '0x0A(LF)'
...
class Form(QWidget):
    ...
    def init_widget(self):
        ...
        현재 위젯의 모양등을 초기화
        """
        self.setWindowTitle("Movon - Nordic Mesh Serial Comm Test Program v0.1")
        ...
        test_data = bytes([0x06]) + bytes([0x02]) +  bytes([0x48]) + bytes([0x65]) + bytes([0x6C]) + bytes([0x6C]) + bytes([0x6F])   # test text 'Hello'
    ...
    def slot_clicked_clear_button(self):
        self.te.clear()
